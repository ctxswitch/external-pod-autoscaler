...
use super::autoscalegroup::{ConditionStatus, MetricCollector, ScrapeConfig};
...
pub struct MetricSpec {
    /// Pod selector for finding scrape targets
    pub pod_selector: PodSelector,

    // Scraper configuration
    pub scraper: MetricScraper,

    /// Label selectors for HPA queries (optional)
    /// If specified, HPA queries must match these selectors
    #[serde(skip_serializing_if = "Option::is_none")]
    pub label_selector: Option<LabelSelector>,
}

/// Metric scraper configuration
...
pub struct MetricScraper {
    ...
    #[schemars(default = "default_aggregation_for_gauge")]
    pub aggregation: AggregationType,
}

impl<'de> Deserialize<'de> for MetricScraper {
        ...
        D: Deserializer<'de>,
    {
        ...
        let helper = MetricScraperHelper::deserialize(deserializer)?;
...
/// Label selector for HPA queries
#[derive(Debug, Clone, Deserialize, Serialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
...
/// Label selector requirement for match expressions
#[derive(Debug, Clone, Deserialize, Serialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
...
pub struct MetricStatus {
    ...
    /// Per-collector aggregated values
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub next_scrape_time: Option<String>,

    /// Metrics collection conditions
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub conditions: Vec<MetricCondition>,
}

/// Per-collector status with aggregated values
#[derive(Debug, Clone, Deserialize, Serialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct CollectorStatus {
    ...
    /// Aggregated values (key: aggregation type, value: computed value)
    pub values: BTreeMap<String, f64>,

    /// Number of successful scrapes for this metric
    #[serde(default)]
...
/// Condition for metrics collection
#[derive(Debug, Clone, Deserialize, Serialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct MetricCondition {
    /// Type of condition
    #[serde(rename = "type")]
    pub type_: MetricConditionType,
...
pub enum MetricConditionType {
    ...
    Operational,
    /// Scraping is degraded (some targets failing)
    Degraded,
    /// All scrape targets are failing
    Failed,
}
